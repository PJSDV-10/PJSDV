 ...
 * 
 * 
 * This is code for the fridge wemos
 * It works like this:
 * 
 * setup {
 *  setupwifi
...
std::string wemosNaam = "fridge1";
std::string server = "Server";
std::string wachtwoord = "solarwinds123";
std::string type = "fridge";
...
unsigned int sensor[AMOUNTOFSENSORS][3] = {{0,0,1},{0,0,300},{0,0,301}}; // sensor array will be {currentvalue,previousvalue, pinnumber} // please put this in te right order otherwise crash
std::string sensorNames[AMOUNTOFSENSORS][2] = {{"bool","switch"},{"int","NTC"},{"int","NTC"}}; // each sensor has a name, but this can't be stored in an int array. {type,name}

unsigned int actuator[AMOUNTOFACTUATORS][3] = {{1,0,16},{1,0,300}}; // actuator array will be {currentvalue, wantedvalue, pinnumber}
std::string actuatorNames[AMOUNTOFACTUATORS][2] = {{"bool","fan"},{"bool","peltier"}}; // each sensor has a name, but this can't be stored in an int array. {type,name} 
...
void loop() {
  ...
  // if any of the sensors changed, we have to notify the server.
    //Serial.println("sending sensorupdate");
     ...
     if ((((sensor[1][0] > 200) && (sensor[1][1] < 200)) || ((sensor[1][0] < 200) && (sensor[1][1] > 200))) || (((sensor[2][0] > 200) && (sensor[2][1] < 200)) || ((sensor[2][0] < 200) && (sensor[2][1] > 200))) || (((sensor[0][0] == 1) && (sensor[0][1] == 0)) || ((sensor[0][1] == 1) && (sensor[0][0] == 0)))) {
